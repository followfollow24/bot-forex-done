...
# -*- coding: utf-8 -*-
"""
preflight_gold_regime.py -- Step 3 MANDATORY pre-flight for the new gold
regime-filter live bot (variant regime22, magic 555103) before it receives
...
live variants (adx20tp7, adx18tp7, btc_cons, btc_aggr, regime22 itself).
...
def main():
    ...
    print("=" * 88)
    print(" GOLD REGIME-FILTER LIVE BOT PRE-FLIGHT (Step 3, mandatory) -- regime22")
    print("=" * 88)
    ...
    # ── check 1: magic number resolves correctly for regime22 ──
    magic = SYMBOL_MAGIC[CANONICAL_SYMBOL] + VARIANT_MAGIC_OFFSET[VARIANT]
    print(f"\nCHECK 1 [magic number]: regime22 -> {magic}  "
          f"(expect 555103 = 555003 base + 100 offset)")
    if magic != 555103:
        print(f"  [FAIL] magic mismatch! got {magic}, expected 555103"); mt5.shutdown(); sys.exit(1)
    print("  [PASS]")
    ...
    if r is None or r.retcode != mt5.TRADE_RETCODE_DONE:
        ...
        close_req = {
            ...
            "comment": "PREFLIGHT-REGIME22-CLOSE"[:24],
            "type_time": mt5.ORDER_TIME_GTC, "type_filling": mt5.ORDER_FILLING_IOC,
        }
        ...
        if cr is None or cr.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            results["order"] = "PASS"
            send_telegram(
                f"[PREFLIGHT TEST] regime22 (XAUUSDc, magic={magic})\n"
    ...
    all_paths = {"regime22": _make_paths(CANONICAL_SYMBOL, VARIANT)}
