...
trend-pullback config. RESEARCH ONLY -- new file, touches nothing live.

Usage:
  python3 _uc_crossasset.py base      # aligned-window unfiltered baselines (BTC + ETH)
  python3 _uc_crossasset.py eth       # ETH-side battery
  python3 _uc_crossasset.py btc       # BTC-side battery
  python3 _uc_crossasset.py follow <variant> [...]   # OOS half-split + neighbors
"""
import sys, os
...
# aligned per-market H1 frames (same rows as M, same order)
...
# ---------------------------------------------------------------- signal arrays (all causal, bar i uses data through bar i close)
...
ratio = se / sb
REMA = {sp: ratio.ewm(span=sp, adjust=False).mean().to_numpy() for sp in (16, 24, 36, 48, 120, 168, 240)}

rb = sb.pct_change(); re_ = se.pct_change()
corr = rb.rolling(720).corr(re_)
...
def gate_ratio_side(fast, slow, side):
    """Gate only one direction; other side ungated. side='L': longs require ratio
    FALLING (EMAfast<EMAslow, the winning inverse direction); side='S': shorts
    require ratio RISING."""
    up = REMA[fast] > REMA[slow]; dn = REMA[fast] < REMA[slow]
    nan = np.isnan(REMA[slow])
    if side == "L":
        return np.where(nan, True, dn).astype(bool), None
    return None, np.where(nan, True, up).astype(bool)
...
VARIANTS = {
    ...
    "btc_ratio36_inv": ("BTC", lambda: gate_ratio(36, 168, inv=True)),
    "btc_r36L":       ("BTC", lambda: gate_ratio_side(36, 168, "L")),
    "btc_r36S":       ("BTC", lambda: gate_ratio_side(36, 168, "S")),
...
NEIGHBORS = {
    ...
    "btc_ratio_inv":  [("r16_168i", lambda: gate_ratio(16, 168, True)), ("r36_168i", lambda: gate_ratio(36, 168, True)),
                       ("r24_120i", lambda: gate_ratio(24, 120, True)), ("r24_240i", lambda: gate_ratio(24, 240, True))],
    "btc_ratio36_inv": [("r24_168i", lambda: gate_ratio(24, 168, True)), ("r48_168i", lambda: gate_ratio(48, 168, True)),
                        ("r36_120i", lambda: gate_ratio(36, 120, True)), ("r36_240i", lambda: gate_ratio(36, 240, True))],
    "btc_r36L":       [("L24_168", lambda: gate_ratio_side(24, 168, "L")), ("L48_168", lambda: gate_ratio_side(48, 168, "L")),
                       ("L36_120", lambda: gate_ratio_side(36, 120, "L")), ("L36_240", lambda: gate_ratio_side(36, 240, "L"))],
    "btc_r36S":       [("S24_168", lambda: gate_ratio_side(24, 168, "S")), ("S48_168", lambda: gate_ratio_side(48, 168, "S")),
                       ("S36_120", lambda: gate_ratio_side(36, 120, "S")), ("S36_240", lambda: gate_ratio_side(36, 240, "S"))],
