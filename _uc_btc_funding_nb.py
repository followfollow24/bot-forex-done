...
variants = {}
for k in (1, 2, 3):
    rise = h1["rise%d" % k].fillna(False).to_numpy(bool)
    fall = h1["fall%d" % k].fillna(False).to_numpy(bool)
    variants["ftrend_fade_d%d" % k] = (~rise, ~fall)   # d2 = original ftrend_fade
...
for name, (ml, ms) in variants.items():
    ...
    print("%-16s %s" % (name, stats(m, tr, yrs)))
print("DONE")
