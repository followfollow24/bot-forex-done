...
# -*- coding: utf-8 -*-
"""
btc_27window_check.py -- apply the SAME 6-month rolling-window harness used
to validate gold ADX20_TP7 (walk_forward_candidates.py / walk_forward_regime.py,
24/27 windows PF>1) to BTC-HF (ADX15/SL4/TP12, real engine). BTC-HF was only
validated with WF-A (8 yearly windows) and WF-B (single split) so far -- this
fills the gap with the finer 6-month-window regime-robustness check.
ASCII-only.
"""
import os, sys, math
import pandas as pd
...
CFG = dict(adx=15, sl=4.0, tp=12.0)   # WF-picked BTC-HF config
...
def main():
    ...
    print("=" * 96)
    print(f" BTC-HF (ADX15/SL4/TP12)  --  6-MONTH ROLLING WINDOWS  ({len(windows)} windows)")
    print(f" same harness as gold ADX20_TP7's 24/27-window validation (walk_forward_candidates.py)")
    print("=" * 96)
    ...
    for i, (w0, w1) in enumerate(windows, 1):
        ...
        bret = gold_return_pct(df, w0, w1)  # reused fn, just price % move of underlying
        reg = regime_tag(bret)
        pf = m["pf"]
        ...
        print(f"  {i:<4} {w0.date()}..{w1.date()}  {bret if bret is not None else 0:>+8.1f}% "
              ...
              f"{ret_pct:>+7.1f}% {m['max_dd_pct']:>6.1f}%{tag}")

    print("  " + "-" * 88)
    print(f"\n  PF>1 in {pf_gt1}/{len(windows)} windows ({n_valid} had trades)")
    print(f"  compare: gold ADX20_TP7 baseline = PF>1 in 24/27 windows (same 6mo methodology)")
    print("=" * 96)
